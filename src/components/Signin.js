...
export default function Signin() {
    ...
    async function handleSubmit(e) {
        ...
        try { ///ใช้ try catch ในการดักจับ error ถ้าไม่ผู้ใช้อยู่ให้ทำการสมัครให้เลย
            setError("")
            setLoading(true)
            await signIn(nameRef?.current?.value, passwordRef?.current?.value) //ถ้า login แล้วรหัสผ่านผิด -> catch 
            navigate('/')
        }
        catch { 
            try {
                await signUp(nameRef?.current?.value, passwordRef?.current?.value)//สมัครให้ใหม่ ระบบจะเช็คว่าเคยมีผู้ใช้นี้อยู่ไหม ถ้ามีแล้ว -> catch
                navigate('/')
            }
            catch {
